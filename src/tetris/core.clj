...
(defn draw-tetrimino [{tetrimino :tetrimino x0 :x y0 :y}]
  (dorun
   (let [[color & coords] (tetriminos tetrimino)]
...
(def current (ref {:tetrimino :o :x 7 :y rows :rotation 0}))

(defn new-tetrimino []
  (alter current assoc
         :tetrimino (rand-nth (keys shapes))
         :x (rand-int (+ 2 (- cols 4)))
         :y rows))
...
(defn setup []
  (dosync (ref-set current {:tetrimino :o :x 7 :y rows}))
  (smooth)
  (background 0 0 0)
  (frame-rate 3))
